...
export function SectorsTable({
  ...
  const { paginatedItems, page, totalPages, total, from, to, search, setSearch, setPage } =
    useClientPagination({
      items: sectors,
      pageSize: 10,
      searchFn: (sector, query) => sector.name.toLowerCase().includes(query),
          ...
          <div className="text-muted-foreground py-8 text-center">
            ...
            {canCreate && (
              <Button asChild variant="outline">
                <Link href="/dashboard/sectors/new">{t('createFirst')}</Link>
              </Button>
            )}
          </div>
        ) : (
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead>{t('table.areas')}</TableHead>
                    ...
                    <TableCell className="text-right">
                      <div className="flex justify-end gap-2">
                          ...
                          <Tooltip>
                            <TooltipTrigger asChild>
                              <Button asChild variant="ghost" size="sm">
                                <Link href={`/dashboard/sectors/${sector.id}/edit`}>
                        ...
                        <Tooltip>
                          <TooltipTrigger asChild>
                            <Button asChild variant="ghost" size="sm">
                              <Link href={`/dashboard/sectors/${sector.id}/staff`}>
                                <Users className="h-4 w-4 text-green-600 dark:text-green-400" />
                              </Link>
                            </Button>
                          </TooltipTrigger>
                          <TooltipContent side="top">{t('table.viewStaff')}</TooltipContent>
                        </Tooltip>
                        {canDelete && (
                          <Tooltip>
                            <TooltipTrigger asChild>
                              <Button
                                ...
                                size="sm"
                                onClick={() =>
                                  setDeleteTarget({ id: sector.id, name: sector.name })
                                }
