...
import { chiefHasAreaAccess, resolveChiefOrganizationId } from '@/src/shared/lib/auth/chief-access'
...
export const addMembersBulkAction = async (
  ...
  try {
    ...
    if (isChiefArea(session)) {
      const hasAccess = await chiefHasAreaAccess(session.id, group.rotation.areaId)
      if (!hasAccess)
        return {
          success: false,
          error: 'Solo puedes gestionar rotativas en las áreas que tienes asignadas',
    ...
    const targetUsers = await prisma.user.findMany({
      where: { id: { in: validatedData.userIds }, organizationId, role: 'STAFF' },
...
export const removeMemberAction = async (
  ...
  try {
    ...
    if (!member) return { success: false, error: 'El usuario no es miembro activo de este grupo' }
