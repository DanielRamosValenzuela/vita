...
export async function OrganizationLimitsCard({
  ...
  const getUsageColor = (usage: (typeof usageSummary.roleUsage)[0]) => {
    if (usage.isOverLimit) return 'destructive'
    if (usage.isAtLimit) return 'destructive'
    if (usage.isNearLimit) return 'warning'
    return 'default'
  }

  const getUsageVariant = (usage: (typeof usageSummary.roleUsage)[0]) => {
    if (usage.isOverLimit) return 'destructive'
    if (usage.isAtLimit) return 'destructive'
